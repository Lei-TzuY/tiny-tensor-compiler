...
- Explicit managed ownership for ordinary serial native artifacts through `ResourceManagedNativeExecutable`, `compile_resource_managed_module()`, and `manage_native_executable()`. Concrete leases share artifact identity/owner accounting with resource-managed dynamic/adaptive handles, support deterministic idempotent `close()` and context-manager release, and unload process staging only when the final managed owner closes or evicts.
- Managed concrete ownership preserves persistent-cache artifacts, survives explicit process-cache clearing by reacquiring on the next open-handle execution, and leaves the wrapped ordinary `NativeExecutable` free to retain its historical reacquire-on-call behavior after the lease closes.
...
- Layout-aware native return materialization and backend eligibility: strided, reversed, or permuted aliases gather into the existing contiguous output ABI and deterministically fall back from flat/SSE2 selection to the general generated-C path unless the resulting layout is actually contiguous.
- Explicit writable-alias `copy_into(root, target, source)` effects for compiler-owned storage, with storage generations tracked independently in tensor IR, Buffer IR, and Loop IR.
...
- Bounded destination-bearing `binary_inplace` effects for exact-typed `add`/`mul` updates of a fresh compiler-owned full storage root from a different storage root, with no extra physical root allocation, explicit storage-generation advancement, direct CPU/C/native mutation, dynamic-specialization and borrowed-source coverage, and serialization-stable operator metadata.
- Verifier-backed partial-region `binary_into` effects for exact-typed `add`/`mul` through injective root-relative target layouts, including signed strides, no destination reallocation, deterministic same-root RHS snapshotting, generation freshness, native GCC/MSVC execution, borrowed-source integration, dynamic specialization, and serialization.
- Exact-dtype broadcast-compatible `copy_into` sources for verified partial target aliases, with target-preserving broadcast inference, canonical Loop IR source maps, explicit CPU/C indexing, deterministic same-root snapshotting, borrowed-input propagation, dynamic specialization, and GCC/MSVC native coverage.
- Explicit `casting="widen"` policy for public `copy_into` / `binary_into` / `add_into` / `mul_into` calls. The bounded exact widening set is `i32->i64`, `i32->f64`, and `f32->f64`; the frontend materializes the RHS through existing typed arithmetic before entering unchanged exact-dtype writable-effect IR.
- Opt-in barriered OpenMP scheduling for verified non-scalar/non-empty `binary_into` effects. Parallel codegen reuses the serial signed-stride write emitter, schedules only the outer target axis, preserves scalar/zero-extent and full-root `binary_inplace` serial behavior, and relies on the existing injective-target plus disjoint/snapshotted-RHS verifier proof rather than adding a second alias-analysis path.
...
- Existing ordinary `NativeExecutable` handles remain non-owning/reacquirable compatibility handles. Managed concrete ownership is opt-in; closing a `ResourceManagedNativeExecutable` invalidates only that lease, while an ordinary wrapped handle may later reacquire the same artifact through the existing native cache path.
- Managed concrete and managed dynamic/adaptive serial handles coordinate one shared process-local ownership identity. `clear_native_cache()` remains an explicit global override, persistent-cache files are never deleted by managed close/eviction, and process-pinned OpenMP executables remain outside reclaimable managed ownership.
...
- Writable effects remain limited to compiler-owned internal roots. Runtime-input and constant roots cannot be mutation roots. High-level same-root `copy_into` and `binary_into` sources use deterministic snapshot-before-write semantics. `binary_inplace` requires a fresh full-root destination, a different-root exact-typed source, and `add` or `mul`; `binary_into` additionally requires an injective partial target layout over the same fresh root. `copy_into` and `binary_into` accept exact-dtype source broadcasting by default. With explicit `casting="widen"`, the frontend accepts only `i32->i64`, `i32->f64`, or `f32->f64`, materializes the source to the destination dtype through existing verified typed arithmetic, and then emits the same exact-dtype writable effect. Narrowing, float-to-int conversion, `i64->f64`, backend-only implicit casts, backend `memmove`, and caller-visible mutable output aliases remain out of scope.
- Optimizers remain deliberately conservative in the presence of writable effects: passes that assume a pure expression graph do not reorder, fold, canonicalize, or CSE across `copy_into`, `binary_into`, or `binary_inplace`, even though verifier-backed fresh generations may be consumed by ordinary computation between ordered writes.
- View aliases never become ordinary writable kernel outputs. Loop verification compares storage roots, tracks storage generations, validates signed layout bounds, and rejects stale handles after a root rewrite; native returns still copy/gather through the established contiguous output ABI rather than exposing internal storage ownership.
- Non-contiguous logical inputs, including negative-stride reversals, do not enter the existing contiguous flat-loop or SSE2 paths. They retain explicit general-C indexing from the verified storage-layout strides; OpenMP may still schedule the general kernel's outer loop.
...
- Parallel native scheduling is opt-in; `parallel=False` retains the serial call shape and code-generation behavior. Scalar and zero-extent kernels/effects remain serial, the existing explicit SSE2 path keeps priority instead of stacking OpenMP onto its vector loop and scalar tail, and full-root `binary_inplace` remains serial.
- OpenMP input materialization and terminal output copies remain serial. Only verified kernel iteration and verifier-safe partial `binary_into` outer loops are scheduled in parallel, and each emitted `parallel for` retains its implicit barrier; this phase does not introduce asynchronous effect execution or graph-level reordering.
