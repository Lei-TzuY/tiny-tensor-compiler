...
- Writable effects remain limited to compiler-owned internal roots. Runtime-input and constant roots cannot be mutation roots. High-level same-root `copy_into` and `binary_into` sources use deterministic snapshot-before-write semantics. `binary_inplace` requires a fresh full-root destination, a different-root exact-typed source, and `add` or `mul`; `binary_into` additionally requires an exact-typed source and an injective partial target layout over the same fresh root. `copy_into` and `binary_into` now accept exact-dtype source broadcasting that resolves exactly to the destination logical shape; destination casting/promotion, backend `memmove`, and caller-visible mutable output aliases remain out of scope.
