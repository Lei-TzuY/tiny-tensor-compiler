...
- `StorageLayout(offset, strides)` descriptors with non-negative root-relative element offsets, strictly positive element strides, concrete root-bound verification, transitive layout composition, and zero-extent-safe canonical contiguous strides.
...
- First-class zero-copy `Tensor.transpose(axes)` views for complete compile-time axis permutations. Shape and positive storage strides are permuted together while root-relative offsets, storage-root lifetime/generation checks, borrowed-input safety, dynamic specialization, and general CPU/C/native indexing remain shared with the existing alias model.
- Layout-aware native return materialization and backend eligibility: strided or permuted aliases gather into the existing contiguous output ABI and deterministically fall back from flat/SSE2 selection to the general generated-C path unless the resulting layout is actually contiguous.
...
- `reshape` remains a verified row-major copy into distinct storage. `view` is the explicit zero-copy alternative for whole-storage C-contiguous shape changes. `slice` adds one compile-time-bounded positive-stride axis and `transpose` adds a complete compile-time axis permutation; all remain read-only aliases. Inferred `-1`, negative strides/reverse views, writable view kernels, runtime permutation axes, and general advanced indexing remain out of scope.
...
- Non-contiguous logical inputs do not enter the existing contiguous flat-loop or SSE2 paths. They retain explicit general-C indexing from the verified storage-layout strides; OpenMP may still schedule the general kernel's outer loop.
