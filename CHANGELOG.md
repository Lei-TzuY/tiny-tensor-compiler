...
- Conservative same-root copy canonicalization for concrete alias regions whose root-relative bounding spans are statically proven disjoint. The frontend materializes the logical source through an explicit owning `reshape` snapshot, so the canonical `copy_into` still consumes a different-root source and existing tensor/Buffer/Loop write invariants remain unchanged.
...
- Writable effects remain limited to compiler-owned internal roots. Runtime-input and constant roots cannot be mutation roots. A concrete same-root source is accepted by the public builder only when root-relative bounding spans prove source and target disjoint; the builder first materializes an explicit source snapshot, so canonical tensor/Buffer/Loop `copy_into` still requires a different-root source. Overlapping or unproven same-root regions, symbolic region proofs, `memmove`, broadcasting, casts, general in-place elementwise kernels, and caller-visible mutable output aliases remain out of scope.
- Optimizers remain deliberately conservative in the presence of writable effects: passes that assume a pure expression graph do not reorder, fold, canonicalize, or CSE across `copy_into`, even though verifier-backed fresh generations may be consumed by ordinary computation between ordered writes.
- View aliases never become writable kernel outputs. Loop verification compares storage roots, tracks storage generations, validates signed layout bounds, and rejects stale handles after a root rewrite; native returns still copy/gather through the established contiguous output ABI rather than exposing internal storage ownership.
