...
- Layout-aware native return materialization and backend eligibility: strided aliases gather into the existing contiguous output ABI and deterministically fall back from flat/SSE2 selection to the general generated-C path.
...
- `reshape` remains a verified row-major copy into distinct storage. `view` is the explicit zero-copy alternative for whole-storage C-contiguous shape changes with identical element-count proofs. `slice` adds one compile-time-bounded positive-stride axis while preserving read-only alias semantics; inferred `-1`, transpose/permutation, negative strides, multi-axis slicing, and writable view kernels remain out of scope.
...
- Strided logical inputs do not enter the existing contiguous flat-loop or SSE2 paths. They retain explicit general-C indexing from the verified storage-layout strides; OpenMP may still schedule the general kernel's outer loop.
