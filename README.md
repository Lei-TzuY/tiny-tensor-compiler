...
-> explicit typed tensor IR (concrete or named runtime symbolic dimensions)
-> verifier
-> optional runtime symbolic binding and concrete specialization
...
- one-or-more named runtime symbolic dimensions on arbitrary axes with exact runtime binding, concrete tensor-IR cloning, and post-specialization verification
...
- NumPy-compatible concrete broadcasting plus conservative symbolic broadcasting (a symbol with itself or dimension `1`; distinct symbols are not implicitly unified)
...
External inputs remain exact and explicit. The ordinary `compile_module()` path requires fully concrete `GraphBuilder.input(shape, dtype)` shapes; runtime calls must match the declared input count, shape, and dtype with no silent cast. `compile_dynamic_module()` adds a bounded specialization boundary: one or more named `SymbolicDim` values may appear on arbitrary input axes and are resolved from runtime shapes, after which the tensor module is cloned, fully concretized, and reverified before entering the existing Buffer/Loop/C11/native pipeline. Every occurrence of the same symbol must agree, every symbol must be runtime-input-bound, and static axes remain exact. Complete bindings such as `B=2,W=3` and `B=2,W=7` cache separate native specializations; a later `B=2,W=3` call reuses the first without making the physical backend itself dynamic.

Copied input materialization remains the default compatibility behavior. Opting into `borrow_inputs()` or `compile_module(..., borrow_inputs=True)` / `compile_dynamic_module(..., borrow_inputs=True)` instead requires exact NumPy, C-contiguous, aligned arrays and binds verified external slots directly; any input that would require a hidden normalization copy is rejected. Arbitrary symbolic arithmetic or affine expressions, implicit equality solving between distinct symbols, named inputs, runtime-sized physical buffers, and reshape-style symbolic transforms are not part of the current dynamic-shape phase.
...
The runtime-input boundary, verifier-backed fusion shapes, persistent native artifact cache, reusable compiled-executable handle, conservative contiguous-loop codegen scheduling, cross-compiler vectorization dependency hint, ordered multi-output ABI, verified zero-copy input binding, complete named runtime-symbol binding specialization, and expression-driven SSE2 selection are now explicit. The next architectural phase should move outward rather than enumerate more shape or opcode/SIMD variants. Candidate frontiers include symbolic shape expressions/constraints, an ISA-neutral vector-plan layer justified by a second executable ISA/backend capability, larger cost-aware expression DAGs, parallel scheduling, and accelerator backends such as CUDA.
