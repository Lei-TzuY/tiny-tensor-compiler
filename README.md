...
The promoted compiler frontier is program-to-program differentiation. The bounded reverse-mode transform now covers static scalar floating losses over `add`, `mul`, `sum`, `reshape`, whole-storage `view`, `transpose`, `reverse`, and positive-stride `slice`. Transpose/reverse adjoints stay zero-copy aliases; slice adjoints deliberately cross into the existing storage-generation/write-effect architecture by scattering through verified `copy_into`, so differentiated programs reuse ordinary Buffer/Loop/native semantics instead of host-side gradient materialization. Symbolic shapes, mixed precision, forward mutation, unsupported operators, and non-scalar selected outputs still fail closed. The next meaningful autodiff depth is executable gradient-consistency evidence rather than operator-name accumulation. A second executable ISA/backend or accelerator backend remains a parallel architectural option only when its toolchain or hardware can be validated.