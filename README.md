...
The promoted compiler frontier is program-to-program differentiation. The current bounded reverse-mode slice transforms static scalar floating losses over `add`, `mul`, `sum`, `reshape`, and whole-storage `view` into ordinary verified tensor IR, including broadcast adjoint reduction and compositional differentiation through the existing primitive matmul lowering. Unsupported symbolic, mixed-precision, alias-scatter, mutation, and non-scalar cases fail closed. The next meaningful autodiff depth should come from executable gradient-consistency evidence or explicit VJP semantics for non-trivial aliases rather than operator-name accumulation. A second executable ISA/backend or accelerator backend remains a parallel architectural option only when its toolchain or hardware can be validated.