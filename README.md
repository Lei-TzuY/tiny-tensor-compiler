...
- explicit `fuse_elementwise()` pass for safe adjacent `add`/`mul` plus ReLU, greedy safe ReLU tails, one safe same-dtype integer `add`/`mul` -> `add`/`mul` chain, and a safe trailing ReLU on that chain when the fused leaf-input alias constraints permit it
