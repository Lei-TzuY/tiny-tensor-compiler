...
-> explicit typed tensor IR (concrete or bounded runtime symbolic/affine dimensions)
-> verifier
...
- concrete typed `input`, `const`, `add`, `mul`, `relu`, and `return` operations plus bounded `SymbolicDim` and one-variable `AffineDim` support in tensor shapes
- one-or-more named runtime symbolic dimensions on arbitrary axes with exact direct/affine runtime solving, concrete tensor-IR cloning, and post-specialization verification
...
- NumPy-compatible concrete broadcasting plus conservative symbolic broadcasting (a symbol with itself, structurally identical affine terms, or dimension `1`; distinct/different symbolic expressions are not implicitly unified)
...
External inputs remain exact and explicit. The ordinary `compile_module()` path requires fully concrete `GraphBuilder.input(shape, dtype)` shapes; runtime calls must match the declared input count, shape, and dtype with no silent cast. `compile_dynamic_module()` adds a bounded specialization boundary: one or more named `SymbolicDim` values may appear on arbitrary input axes, either directly or inside one-variable positive affine terms such as `2*B+1`. A direct occurrence binds the runtime extent to the symbol; an affine occurrence solves `(extent-offset)/scale` and requires a non-negative exactly divisible residual. Every occurrence of the same symbol must agree, every symbol must be runtime-input-bound, and static axes remain exact. The tensor module is then cloned, fully concretized, reverified, and only then enters the existing Buffer/Loop/C11/native pipeline. Complete bindings such as `B=2,W=3` and `B=2,W=7` cache separate native specializations; a later `B=2,W=3` call reuses the first without making the physical backend itself dynamic.

Copied input materialization remains the default compatibility behavior. Opting into `borrow_inputs()` or `compile_module(..., borrow_inputs=True)` / `compile_dynamic_module(..., borrow_inputs=True)` instead requires exact NumPy, C-contiguous, aligned arrays and binds verified external slots directly; any input that would require a hidden normalization copy is rejected. Multi-variable symbolic expressions, subtraction/division, implicit equality solving between distinct symbols, named inputs, runtime-sized physical buffers, and reshape-style symbolic transforms are not part of the current bounded dynamic-shape phase.
...
The runtime-input boundary, verifier-backed fusion shapes, persistent native artifact cache, reusable compiled-executable handle, conservative contiguous-loop codegen scheduling, cross-compiler vectorization dependency hint, ordered multi-output ABI, verified zero-copy input binding, complete named runtime-symbol binding specialization, bounded one-variable affine runtime constraints, and expression-driven SSE2 selection are now explicit. The next architectural phase should move outward rather than enumerate more affine/shape or opcode/SIMD variants. Candidate frontiers include a richer uniquely-solvable relational shape constraint system, an ISA-neutral vector-plan layer justified by a second executable ISA/backend capability, larger cost-aware expression DAGs, parallel scheduling, and accelerator backends such as CUDA.
