...
- explicit `fuse_elementwise()` pass for safe adjacent `add`/`mul` plus ReLU, greedy safe ReLU tails, one safe same-dtype integer `add`/`mul` -> `add`/`mul` chain, a safe trailing ReLU on that chain when the fused leaf-input alias constraints permit it, and one adjacent branch-shaped same-dtype integer binary tree with one safe trailing ReLU when tree-output liveness and leaf-alias constraints permit it
