...
The promoted compiler frontier is program-to-program differentiation. The bounded reverse-mode transform covers static scalar floating losses over `add`, `mul`, `sum`, `reshape`, whole-storage `view`, `transpose`, `reverse`, and positive-stride `slice`. Transpose/reverse adjoints stay zero-copy aliases; slice adjoints cross into the existing storage-generation/write-effect architecture through verified `copy_into`. A deterministic smooth-graph gradient-consistency campaign now checks analytic transforms against central finite differences and shrinks reproducible divergences into canonical repro artifacts. Symbolic shapes, mixed precision, forward mutation, unsupported operators, and non-scalar selected outputs still fail closed. The next major autodiff frontier is semantic expansion across the symbolic/dynamic shape boundary, not more static operator-name accumulation. A second executable ISA/backend or accelerator backend remains a parallel architectural option only when its toolchain or hardware can be validated.