...
A native C compiler is required to exercise `execute_native()`, `compile_native()`, or a newly observed `compile_dynamic_module()` specialization: a `cc`-compatible GCC/Clang toolchain on POSIX-like systems, or an MSVC developer environment exposing `cl` on Windows. Parallel native execution additionally requires that selected toolchain's OpenMP compiler/runtime support (`-fopenmp` or `/openmp`). CI executes the full suite, including the OpenMP native regressions, on Ubuntu and Windows for Python 3.11 and 3.13.

## Near-term compiler roadmap

The runtime-input boundary, persistent native artifact cache, reusable compiled-executable handle, conservative contiguous-loop scheduling, cross-compiler vectorization hints, ordered multi-output ABI, verified zero-copy input binding, exact relational symbolic-shape specialization, first-class fused expressions, bounded cost-ranked generic DAG fusion, expression-driven SSE2 selection, and verified opt-in native OpenMP loop scheduling are now explicit. Raising the fusion-node limit or micro-tuning OpenMP thread/grain settings without controlled benchmark evidence would be low-value farming. The next phase should add a genuinely new executable compiler layer, with leading candidates including a reshape/shape-transform subsystem that creates new symbolic-shape semantics, a second genuinely executable ISA/backend that can justify an ISA-neutral vector plan, or an accelerator backend such as CUDA.