...
-> explicit typed tensor IR (concrete or one leading symbolic batch dimension)
-> verifier
-> optional runtime batch specialization to concrete tensor IR
...
- concrete typed `input`, `const`, `add`, `mul`, `relu`, and `return` operations plus bounded `SymbolicDim` support in tensor shapes
- one shared leading runtime symbolic batch dimension with explicit runtime binding, concrete tensor-IR cloning, and post-specialization verification
- reusable `DynamicExecutable` handles that freeze the symbolic template and cache one ordinary `NativeExecutable` per concrete batch size
...
- NumPy-compatible concrete broadcasting plus conservative symbolic broadcasting (`B` with the same `B` or dimension `1`)
...
External inputs remain exact and explicit. The ordinary `compile_module()` path requires fully concrete `GraphBuilder.input(shape, dtype)` shapes; runtime calls must match the declared input count, shape, and dtype with no silent cast. `compile_dynamic_module()` adds one deliberately bounded exception: a shared leading `SymbolicDim("B")` may be resolved from runtime inputs, after which the tensor module is cloned, concretized, and reverified before entering the existing Buffer/Loop/C11/native pipeline. Every occurrence of `B` must agree, while all other axes remain exact. The same symbolic module may therefore reuse separate native specializations for `B=2`, `B=7`, and later `B=2` again without making the physical backend itself dynamic.

Copied input materialization remains the default compatibility behavior. Opting into `borrow_inputs()` or `compile_module(..., borrow_inputs=True)` / `compile_dynamic_module(..., borrow_inputs=True)` instead requires exact NumPy, C-contiguous, aligned arrays and binds verified external slots directly; any input that would require a hidden normalization copy is rejected. Arbitrary symbolic arithmetic, multiple independent symbols, non-leading symbolic axes, named inputs, runtime-sized physical buffers, and reshape-style shape transforms are not part of the current dynamic-batch phase.
...
The runtime-input boundary, verifier-backed fusion shapes, persistent native artifact cache, reusable compiled-executable handle, conservative contiguous-loop codegen scheduling, cross-compiler vectorization dependency hint, ordered multi-output ABI, verified zero-copy input binding, and one-leading-symbol runtime batch specialization are now explicit. The next architectural phase should move outward rather than enumerate more batch-shape or opcode/SIMD variants. Candidate frontiers include a generalized SIMD abstraction, broader symbolic-shape algebra, general expression-DAG matching, parallel scheduling, and accelerator backends such as CUDA.
