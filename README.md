...
The promoted compiler frontier is program-to-program differentiation. The bounded reverse-mode transform covers static scalar floating losses over `add`, `mul`, `sum`, `reshape`, whole-storage `view`, `transpose`, `reverse`, and positive-stride `slice`. Transpose/reverse adjoints stay zero-copy aliases; slice adjoints cross into the existing storage-generation/write-effect architecture through verified `copy_into`. A deterministic smooth-graph gradient-consistency campaign checks analytic transforms against central finite differences and shrinks reproducible divergences into canonical repro artifacts. Dynamic gradients now cross the symbolic/runtime boundary without teaching the AD transform unresolved-shape semantics: `compile_dynamic_gradient_module()` first binds and specializes the symbolic forward module to exact concrete shapes, then differentiates and compiles that concrete gradient module, caching one native specialization per runtime binding. Mixed precision, forward mutation, unsupported operators, and non-scalar selected outputs still fail closed after specialization. The next meaningful autodiff frontier is deeper dynamic policy—multi-symbol/adaptive gradient specialization—or an explicit higher-order contract, not more static operator-name accumulation. A second executable ISA/backend or accelerator backend remains a parallel architectural option only when its toolchain or hardware can be validated.