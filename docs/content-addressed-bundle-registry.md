...
An optional, separate Ed25519 publisher-authorization layer is available for both ordinary dynamic bundle archives and retained-linearization archives. Ordinary bundles use `publish_attested_dynamic_bundle_set_archive()`, `fetch_attested_dynamic_bundle_set_archive()`, and `load_attested_dynamic_bundle_set_registry()`; retained payloads use the corresponding `*_dynamic_linearization_bundle_set_*` APIs. Both require a caller-pinned `PublisherTrustPolicy`, verify a detached signature over the exact archive digest, fail unknown/revoked publishers before network access, keep fetched bytes in private staging until authorization succeeds, and preserve payload-specific archive verification before compiler-free execution. See `bundle-publisher-attestations.md` for the key, revocation, signature, fail-closed staging, and threat boundaries.
...
Publisher-attestation parity is now complete for retained-linearization archives without a payload-specific trust protocol. The next retained-linearization deployment step is release-channel freshness/rollback parity: bind an already signed channel checkpoint to an attested retained archive, reject rollback from caller-owned state before archive/attestation download, and load the coordinated linearization executable only after the accepted floor is persisted. Threshold policy and transparency remain later, separately verifiable properties.
