...
The reverse-mode writable-effect family and dynamic VJP/HVP stack have bounded executable contracts and managed lifecycle parity, while forward mode now spans static JVP transformation plus native and adaptive dynamic specialization across symbolic runtime bindings. Unsupported write effects remain fail-closed in forward mode rather than being farmed as variants. The next architectural gap is resource-managed dynamic JVP retention/release parity: native and adaptive JVP specializations should enter the same deterministic LRU ownership, eviction, artifact-release, and native-reacquisition lifecycle already shared by ordinary dynamic, gradient, VJP, and HVP executables.