...
The autodiff phase now has source-to-source scalar gradients, runtime-seeded tensor-output VJPs, executable reverse-over-reverse Hessian-vector products, direct-slice `copy_into` higher-order semantics, generation-aware pre-write primal tape across ordered effects, and direct-slice `binary_into` VJPs for exact floating `add`/`mul`. For `add`, the pre-write root receives the full output cotangent and the source receives the gathered written region. For `mul`, the written root region receives `cotangent * source`, while the source receives `cotangent * taped_prewrite_target` followed by the existing unbroadcast rule. These paths reuse ordinary tensor IR plus the same verified partial-write machinery. The next architectural gap is the full-root `binary_inplace` reverse contract; additional partial-target variants remain out of scope.