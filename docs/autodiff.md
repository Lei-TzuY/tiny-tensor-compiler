...
The reverse-mode writable-effect family and dynamic VJP/HVP stack have bounded executable contracts and managed lifecycle parity, while forward mode spans static JVP transformation, native/adaptive/resource-managed dynamic specialization, the same writable-effect family, and now independent directional-finite-difference plus JVP/VJP-duality evidence across deterministic smooth graphs. More mutation-target variants, cache wrappers, or ad-hoc Jacobian materialization would now be low-value. The next architectural gap is first-class linearization: construct a joint primal-plus-derivative program that preserves one verified primal evaluation and exposes reusable pushforward and/or pullback execution for multiple tangent or cotangent queries, rather than rebuilding transformed modules and recomputing the primal for every derivative request.