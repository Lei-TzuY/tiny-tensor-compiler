...
The current writable-effect family has bounded reverse semantics for direct-slice `copy_into`, direct-slice `binary_into add/mul`, and full-root `binary_inplace add/mul`, all reusing ordinary tensor IR, verified storage generations, and one pre-write primal tape. Runtime-seeded VJPs now cross symbolic specialization through native and adaptive dynamic execution and share the same managed lifecycle as ordinary and gradient specializations. Non-direct partial targets and unsupported operators remain fail-closed rather than being farmed as alias variants. The next architectural gap is a bounded dynamic Hessian-vector-product executable: solve symbols from the original forward inputs, specialize the scalar-loss forward module, construct its single-input gradient, apply a runtime-seeded VJP to that concrete gradient, and cache the resulting HVP by the original complete binding while treating the appended vector as ABI data rather than a source of symbolic bindings.