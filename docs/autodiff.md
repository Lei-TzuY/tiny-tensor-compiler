...
The generation-aware retained-write contract now covers direct positive-stride slice `copy_into`, direct-slice `binary_into add/mul`, and full-root `binary_inplace add/mul`. Additive writes reuse exact tangent/cotangent generation routing without retaining unnecessary multiplicative primal state. Multiplicative partial writes retain pre-write target/source primals, while multiplicative full-root writes retain the pre-write root/source primals required by product rules before storage generation advances. Reusable pushforward, reusable pullback, and shared linearization consume one deterministic tape layout across all of these supported generations. Non-direct partial targets remain fail-closed. No mutable alias handle is retained across query lifetimes: only immutable owned tensor snapshots and the retained primal result cross the boundary.

### Adaptive dynamic retained-state admission

`compile_adaptive_dynamic_linearization()` preserves the dynamic retained-state ordering while applying one coherent structural-admission decision to the concrete retained bundle. Primal inputs alone solve symbolic bindings; the forward module is specialized first; then the concrete primal/tape, pushforward, and pullback modules are constructed and checked against the same `CompileBudget`. If every component is admitted, all three compile natively. If any component exceeds the structural budget, all three execute through verified Loop IR. The bundle records the first exceeded component for evidence, and each complete binding caches one stable backend decision. Tangent and cotangent query inputs remain state-local ABI data and never participate in symbolic inference.

### Resource-managed retained-linearization lifecycle

Dynamic and adaptive retained-linearization specializations now reuse the existing deterministic LRU ownership registry while allowing one logical binding to own a set of native artifacts. Native retained bundles enumerate their primal/tape, pushforward, and pullback executables; each exact artifact identity is retained and released independently, so duplicate identities within or across managed handles preserve reference counts and unload only on final managed ownership. Adaptive Loop bundles enumerate no native artifacts and therefore advance only specialization-eviction accounting. `released_native_artifact_count` remains an actual unload counter, so one evicted native retained bundle may increase it by more than one while a shared bundle may increase it by zero until the final owner releases those identities. Externally retained concrete linearization executables keep their existing reacquire-on-use behavior after managed eviction.

### Compiler-free retained-linearization bundle sets

Finite symbolic retained-linearization families can now be packaged as `native-linearization-bundle-set-v1`. Each concrete binding owns three canonical child roles—primal/tape, pushforward, and pullback—built through the existing verified native-bundle format. The outer manifest binds the symbolic input template, `wrt` ordering, binding, tape count, canonical component paths, child-manifest SHA-256 values, and child ABI SHA-256 values. Loading recomputes the concrete primal input ABI and validates the cross-component retained-state contract before any derivative query: primal outputs determine the tape ABI, pushforward inputs must be retained inputs + tape + tangents, and pullback inputs must be retained inputs + tape + selected-output cotangent. Runtime primal inputs dispatch only to packaged bindings, child libraries load lazily, and the returned state reuses the existing caller-mutation-isolated `LinearizationState`.

### Deterministic retained-linearization archives

Verified finite retained-linearization bundle sets can now be wrapped in the existing `native-bundle-archive-v1` single-file transport under payload kind `retained-linearization-bundle-set`. Packing eagerly revalidates every binding's coordinated primal/tape, pushforward, and pullback children before publication, writes canonical sorted `ZIP_STORED` entries with fixed metadata, validates the exact temporary archive before atomic rename, and rejects canonical-equivalent path aliases in addition to traversal, duplicate, symlink, encryption, compression, and malformed-schema cases. Loading extracts only after validating the complete entry set, eagerly revalidates the multi-artifact retained-state ABI, then returns a fresh compiler-free dispatcher whose child components remain lazily loaded for execution. Closing the archive executable unloads loaded components before removing the private extraction tree.

### Content-addressed retained-linearization registry

Retained-linearization archives now cross the existing unsigned content-addressed registry without a second HTTP or staging implementation. Publication validates the local retained archive before upload, addresses the exact archive bytes by canonical SHA-256, reads the immutable object back, rechecks the pinned digest, and dispatches the remote bytes through the retained-linearization archive verifier. Fetch and direct load reuse the existing transfer limit, Content-Length validation, redirect refusal, bearer-token boundary, private staging, and atomic destination publication. The registry wrapper owns both the private downloaded archive and the extracted retained payload until close, while the returned linearization state preserves compiler-free primal, pushforward, and pullback execution. Ordinary dynamic bundle archives and retained-linearization archives therefore share one byte-integrity transport while retaining payload-specific verification and fail-closed wrong-kind handling.

### Publisher-attested retained-linearization registry

Retained-linearization archives now share the existing caller-pinned Ed25519 authorization layer instead of introducing a second trust protocol. Publication signs the exact retained archive digest and re-reads the immutable attestation object; fetch rejects unknown or revoked publishers before network access, downloads the retained archive through the existing digest-pinned verifier into private staging, verifies the exact publisher/digest signature, and only then atomically publishes the caller destination. Direct registry load returns a compiler-free coordinated linearization executable carrying both the verified digest and publisher identity. Wrong payload kinds and substituted attestations remain fail-closed before caller-visible execution.

## Next architectural frontier

Generation-aware retained snapshots, native/adaptive dynamic retained-state specialization, multi-artifact lifecycle ownership, compiler-free retained-linearization bundle sets, deterministic archive transport, digest-pinned registry delivery, and publisher-attested retained registry loading are now closed under bounded executable contracts. The next deployment-security gap is retained-linearization release-channel rollback/freshness parity: reuse the existing signed channel checkpoint and caller-owned `ReleaseStateStore`, perform rollback precheck before archive or attestation requests, fetch the publisher-attested retained archive, then persist the accepted floor before exposing the coordinated linearization executable. Threshold policy and transparency remain later layers until this rollback-protected retained payload path is executable and fail-closed. Borrowed retained inputs and performance claims remain separate later concerns.