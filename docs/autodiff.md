...
Pushforward and pullback currently share the same pure-graph validation and nonlinear tape-value selection, but still own separate state/executable wrappers and separate primal/tape module construction. Write effects remain fail-closed in both retained-state directions until generation-aware snapshots can safely outlive the one-shot primal execution.

## Next architectural frontier

The compiler now has symmetric reusable pure pushforward and pullback states: each evaluates the primal once, freezes the required retained values, and services multiple tangent or cotangent queries without replaying the forward graph. The next architectural gap is convergence, not another derivative variant: build one first-class linearization tape/state with one primal execution and one retained-value layout that can expose both pushforward and pullback query paths. Generation-aware writable-effect snapshots should enter only after that shared lifetime/ownership contract is explicit and verifier-safe.