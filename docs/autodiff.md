...
The generation-aware retained-write contract now covers direct positive-stride slice `copy_into` and direct-slice `binary_into add/mul`. Additive partial writes reuse tangent/cotangent generation routing without retaining unnecessary multiplicative primal state. Multiplicative partial writes retain the exact pre-write target/source primals required by product rules before storage generation advances, and reusable pushforward, reusable pullback, and shared linearization consume the same deterministic tape layout. Non-direct partial targets and full-root arithmetic writes remain fail-closed. No mutable alias handle is retained across query lifetimes: only immutable owned tensor snapshots and the retained primal result cross the boundary.

## Next architectural frontier

Generation-aware direct-copy and partial-arithmetic retention are now closed across reusable pushforward, reusable pullback, and the shared linearization state. The next architectural gap is full-root `binary_inplace add/mul` retained snapshots under the same one-tape lifetime protocol. The additive case should preserve exact full-root tangent/cotangent routing without unnecessary tape values; the multiplicative case must retain the pre-write root/source primals needed after generation advance. Non-direct partial targets should remain fail-closed rather than becoming alias-variant farming. Symbolic retained-state specialization, borrowed retained inputs, and performance claims remain separate later concerns.