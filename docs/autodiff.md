...
The reverse-mode writable-effect family and dynamic VJP/HVP stack have bounded executable contracts and managed lifecycle parity, while forward mode now spans static JVP transformation, native/adaptive/resource-managed dynamic specialization, generation-aware direct-slice `copy_into`, and direct-slice `binary_into add/mul` across reference, CPU, Loop, and native execution. The next architectural gap is the full-root `binary_inplace` forward contract. The bounded next slice should reuse the existing pre-write primal snapshot discipline: `add` should send `root_tangent + source_tangent` to the new root tangent, while `mul` should produce `root_tangent * source_primal + taped_prewrite_root * source_tangent`. Partial-target variants and new cache policies should not be expanded further before that full-root effect is executable and verified.