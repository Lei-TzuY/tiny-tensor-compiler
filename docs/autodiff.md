...
Generation-aware retained snapshots are closed across the currently exposed writable-effect family, native dynamic retained-state specialization is complete, and adaptive dynamic retained-state bundles now select one coherent native-or-Loop backend after concrete specialization. The next architectural gap is resource-managed retained-linearization lifecycle parity. Unlike ordinary dynamic gradient/VJP/HVP/JVP specialization, one retained-linearization binding may own three coordinated native executables (primal tape, pushforward, pullback), so eviction must retain/release all artifact identities as one logical binding, preserve final-owner unload semantics for shared identities, keep external executable reacquisition valid, and treat adaptive Loop bundles as eviction-only decisions with no false native-release accounting. Borrowed retained inputs and performance claims remain separate later concerns.