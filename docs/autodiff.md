...
The current writable-effect family now has bounded reverse semantics for direct-slice `copy_into`, direct-slice `binary_into add/mul`, and full-root `binary_inplace add/mul`, all reusing ordinary tensor IR, verified storage generations, and one pre-write primal tape. Non-direct partial targets and unsupported operators remain fail-closed rather than being farmed as alias variants. Runtime-seeded VJPs now also cross symbolic specialization through `DynamicVJPExecutable`: forward inputs alone solve the binding, the concrete selected output determines the appended cotangent ABI, and repeated bindings reuse the same native VJP specialization. The next architectural gap is adaptive dynamic VJP admission/fallback so each concrete VJP can reuse the existing structural budget decision and verified Loop fallback rather than introducing another execution policy.