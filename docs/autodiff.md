...
This is intentionally not yet a reusable linearization closure: each new tangent query executes the joint program again and therefore recomputes the primal. No claim is made that primal intermediates survive across calls.

## Next architectural frontier

The reverse-mode writable-effect family and dynamic VJP/HVP stack have bounded executable contracts and managed lifecycle parity, while forward mode now includes static/dynamic JVPs, the same writable-effect family, independent finite-difference/duality evidence, and a verified joint value-plus-JVP program sharing one primal graph per execution. More mutation-target variants, cache wrappers, or ad-hoc Jacobian materialization would now be low-value. The next architectural gap is reusable first-class linearization: evaluate the primal once, retain the exact intermediates required by a pushforward and/or pullback, and service multiple tangent or cotangent queries without rebuilding transformed modules or recomputing the primal.