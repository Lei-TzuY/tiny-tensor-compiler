...
Generation-aware retained snapshots are closed across the currently exposed writable-effect family, native and adaptive dynamic retained-state specialization are complete, and retained-linearization bundles now have deterministic multi-artifact lifecycle ownership. The next architectural gap is compiler-free retained-linearization packaging: represent one complete symbolic binding as a coordinated primal/tape + pushforward + pullback native bundle, preserve the shared input/tape ABI and retained-state semantics without a compiler at deployment time, and integrate that multi-artifact binding with the existing deterministic native bundle-set/archive verification boundary. Trust, signing, release-channel, and transparency promotion should reuse the existing bundle infrastructure only after the new multi-artifact manifest/ABI contract is executable and fail-closed. Borrowed retained inputs and performance claims remain separate later concerns.