...
The autodiff phase now has source-to-source scalar gradients, runtime-seeded tensor-output VJPs, executable reverse-over-reverse Hessian-vector products, direct-slice `copy_into` higher-order semantics, generation-aware pre-write primal tape across ordered copy effects, backend differential regressions, independent finite-difference consistency evidence, lazy runtime symbolic specialization, multi-symbol gradient caching, per-binding adaptive native/Loop policy, and resource-managed gradient retention. The next architectural gap is explicit reverse semantics for value-updating arithmetic write effects such as `binary_into` and `binary_inplace`, where root/source cotangents depend on the written operator and taped target primals, rather than additional copy-target variants.