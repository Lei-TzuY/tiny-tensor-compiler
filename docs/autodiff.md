...
The current writable-effect family has bounded reverse semantics for direct-slice `copy_into`, direct-slice `binary_into add/mul`, and full-root `binary_inplace add/mul`, all reusing ordinary tensor IR, verified storage generations, and one pre-write primal tape. Runtime-seeded VJPs and bounded single-input HVPs cross native and adaptive dynamic specialization and now share the same managed lifecycle as ordinary and gradient specializations. Non-direct partial targets, unsupported operators, multi-input/block Hessians, and full Hessian materialization remain fail-closed or out of scope rather than being farmed as variants. The next architectural gap is forward-mode Jacobian-vector products: introduce a bounded source-to-source tangent transform over the proven pure floating subset, append exact-type runtime tangent inputs for selected primal inputs, propagate primal/tangent pairs through ordinary tensor IR, and validate the result across reference, CPU, Loop, and native execution without adding a forward-mode backend opcode.