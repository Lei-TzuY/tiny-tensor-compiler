...
Generation-aware retained snapshots, native/adaptive dynamic retained-state specialization, multi-artifact lifecycle ownership, compiler-free retained-linearization bundle sets, and deterministic single-file retained-linearization archive transport are now closed under bounded executable contracts. The next architectural gap is content-addressed registry integration for this payload kind: reuse the existing digest-pinned staging/download limits and archive SHA-256 addressing, but dispatch verified retained-linearization bytes through the retained-linearization archive loader and expose the coordinated linearization executable. Publisher authorization, release-channel rollback/freshness, threshold policy, and transparency promotion remain later layers until the unsigned digest-pinned retained-linearization registry path is executable and fail-closed. Borrowed retained inputs and performance claims remain separate later concerns.