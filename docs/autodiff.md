...
Generation-aware retained snapshots, native/adaptive dynamic retained-state specialization, multi-artifact lifecycle ownership, and compiler-free finite retained-linearization bundle sets are now closed under bounded executable contracts. The next architectural gap is deterministic single-file retained-linearization archive packaging/loading: reuse the existing archive transport rules—canonical entry names, no symlinks, stored compression, atomic publication, private extraction, eager payload verification, and cleanup—while teaching that boundary about the new multi-artifact bundle-set kind. Trust, signing, registry, release-channel, and transparency promotion remain later layers and should not be extended until the archive itself is executable and fail-closed. Borrowed retained inputs and performance claims remain separate later concerns.