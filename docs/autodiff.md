...
The reverse-mode writable-effect family and dynamic VJP/HVP stack have bounded executable contracts and managed lifecycle parity, while forward mode now spans static JVP transformation plus native, adaptive, and resource-managed dynamic specialization across symbolic runtime bindings. The remaining architectural gap is generation-aware forward-mode write-effect differentiation. The next bounded slice should start with direct-slice `copy_into`: the primal path advances the writable root generation exactly as today, while the tangent path must preserve untouched tangent elements and overwrite the target region with the source tangent using verified ordinary tensor/write IR. Non-direct targets and arithmetic writes should remain fail-closed until that first write-effect JVP is executable across the existing backends.