...
`PushforwardLinearizationState` therefore services multiple `pushforward()` calls after one `linearize()` call. Caller-owned primal arrays are copied into retained state, so later mutation of those arrays cannot change derivative queries. The state intentionally holds the pushforward executable and retained tensors but not the primal executable; subsequent tangent queries have no execution path that recomputes the primal. This first slice is static and pure-only: `copy_into`, `binary_into`, and `binary_inplace` remain fail-closed because retaining verifier-safe generation snapshots across query lifetimes is a separate architectural problem. Borrow-retained inputs, symbolic specialization, and performance claims are also outside this contract.

### Reusable pullback linearization

`compile_pullback_linearization()` mirrors the retained-state pushforward contract for reverse mode on the same static pure floating subset. Its one-shot primal/tape module evaluates the selected primal once and returns only the nonlinear intermediate primals required by multiplication adjoints. `PullbackLinearizationState` freezes owned copies of the original inputs and tape values, exposes the retained primal, and accepts repeated runtime cotangents through `pullback()`; the reusable reverse program propagates adjoints directly from those retained values and does not clone or execute the original primal arithmetic/reduction chain.

Pushforward and pullback use the same pure-graph validation and deterministic nonlinear tape-value selection. `compile_linearization()` now validates that the independently constructed pushforward and pullback primal/tape modules are identical, then retains only one primal/tape executable and one frozen retained-value layout. `LinearizationState` exposes both derivative directions from that shared state, owns copies of caller-provided primal inputs and retained tape tensors, and intentionally does not retain the primal executable; repeated tangent and cotangent queries therefore cannot replay the original forward graph.

The shared state remains static and pure-only. `copy_into`, `binary_into`, and `binary_inplace` still fail closed because retaining generation-specific snapshots past the one-shot primal execution requires an explicit lifetime/ownership rule, not merely another derivative formula.

## Next architectural frontier

Pure retained-state convergence is now complete: one primal execution and one retained-value layout can service both pushforward and pullback queries. The next architectural gap is generation-aware writable-effect retention. Extend the shared tape with verifier-safe snapshots for the exact storage generations required by bounded write-effect JVP/VJP rules, prove those snapshots remain immutable and unambiguous across repeated query lifetimes, and only then admit `copy_into` / arithmetic-write linearization. Symbolic retained-state specialization, borrowed retained inputs, and performance claims remain separate later concerns.