...
The current writable-effect family now has bounded reverse semantics for direct-slice `copy_into`, direct-slice `binary_into add/mul`, and full-root `binary_inplace add/mul`, all reusing ordinary tensor IR, verified storage generations, and one pre-write primal tape. Runtime-seeded VJPs cross symbolic specialization through both native and adaptive dynamic execution without letting the cotangent participate in symbol solving, and adaptive admission is evaluated on each concrete VJP rather than on the symbolic forward graph. Non-direct partial targets and unsupported operators remain fail-closed rather than being farmed as alias variants. The next architectural gap is resource-managed dynamic VJP retention/release parity: native and adaptive VJP specializations should enter the same deterministic LRU ownership, eviction, artifact-release, and native-reacquisition lifecycle already shared by ordinary dynamic and dynamic-gradient executables.