...
Generation-aware retained snapshots are now closed across the currently exposed writable-effect family for reusable pushforward, reusable pullback, and the shared `LinearizationState`. The next architectural gap is symbolic/dynamic retained-state specialization: solve complete runtime bindings from the original primal inputs, specialize the forward module first, construct one concrete retained primal/tape plus reusable pushforward/pullback executables for that binding, and keep tangent/cotangent query inputs out of symbolic inference. The initial slice should preserve the current owned retained-state lifetime contract and deterministic tape layout; borrowed retained inputs, adaptive admission, lifecycle retention limits, and performance claims remain separate later concerns.