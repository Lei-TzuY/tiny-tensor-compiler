...
The reverse-mode writable-effect family and dynamic VJP/HVP stack have bounded executable contracts and managed lifecycle parity, while forward mode now spans both static JVP transformation and native dynamic specialization across symbolic runtime bindings. Unsupported write effects remain fail-closed in forward mode rather than being farmed as variants. The next architectural gap is adaptive dynamic JVP admission/fallback: apply the existing structural budget to each concrete JVP after primal specialization and tangent-ABI construction, then cache one stable native-or-verified-Loop backend decision per original symbolic binding.