...
The reverse-mode writable-effect family and dynamic VJP/HVP stack now have bounded executable contracts and managed lifecycle parity, while forward mode has a verifier-backed static JVP transform across the proven pure floating subset. The next architectural gap is dynamic JVP specialization: solve symbolic bindings from the original primal runtime inputs only, specialize the forward module first, construct the concrete JVP whose appended tangent ABI exactly matches the selected specialized `wrt` inputs, and cache the resulting executable by the original complete binding without letting tangent inputs participate in symbolic inference.