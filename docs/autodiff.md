...
The compiler now has joint primal+JVP execution and a real reusable pure pushforward state that evaluates the primal once and answers multiple tangent queries from retained nonlinear intermediates. Repeating write-target variants, lifecycle wrappers, or ad-hoc Jacobian materialization would now be low-value. The next architectural gap is reusable pullback linearization: retain the exact primal values required by reverse mode once, accept multiple cotangent queries without replaying the forward graph, and then converge pushforward and pullback onto one shared first-class linearization tape. Generation-aware writable-effect snapshots should enter only when that retained-state contract can preserve verifier/storage-generation invariants across query lifetimes.