...
The reverse-mode writable-effect family and dynamic VJP/HVP stack have bounded executable contracts and managed lifecycle parity, while forward mode now spans static JVP transformation, native/adaptive/resource-managed dynamic specialization, and a generation-aware direct-slice `copy_into` tangent rule across the existing backends. The next architectural gap is arithmetic write-effect JVP semantics. The next bounded slice should start with direct-slice `binary_into`: `add` should preserve the untouched root tangent while writing `target_tangent + source_tangent` into the updated region, and `mul` should write the ordinary product rule `target_tangent * source_primal + target_primal * source_tangent`, which requires a verifier-safe pre-write primal target snapshot before the primal generation advances. Non-direct targets and full-root `binary_inplace` should remain fail-closed until that partial arithmetic-write rule is executable and verified.