...
Generation-aware retained snapshots, native/adaptive dynamic retained-state specialization, multi-artifact lifecycle ownership, compiler-free retained-linearization bundle sets, deterministic archive transport, and unsigned digest-pinned registry delivery are now closed under bounded executable contracts. The next deployment-security gap is publisher-attested retained-linearization registry parity: reuse the existing Ed25519 digest authorization and pinned PublisherTrustPolicy, but verify/stage/load through the retained-linearization registry path and expose the coordinated linearization executable only after both byte identity and publisher authorization succeed. Release-channel rollback/freshness, threshold policy, and transparency remain later layers until this publisher-authenticated retained payload path is executable and fail-closed. Borrowed retained inputs and performance claims remain separate later concerns.