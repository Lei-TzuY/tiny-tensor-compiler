...
The reverse-mode writable-effect family and dynamic VJP/HVP stack have bounded executable contracts and managed lifecycle parity, while forward mode now spans static JVP transformation, native/adaptive/resource-managed dynamic specialization, direct-slice `copy_into`, direct-slice `binary_into add/mul`, and full-root `binary_inplace add/mul` across reference, CPU, Loop, and native execution. With the current writable-effect family covered under bounded forward-mode contracts, the next architectural gap is independent JVP correctness evidence rather than more mutation variants: a deterministic smooth-graph campaign should compare analytic JVPs against central directional finite differences, and a duality campaign should verify `dot(J·v, c) == dot(v, Jᵀ·c)` by composing the existing JVP and runtime-seeded VJP transforms on the same verified graph. Failures should produce deterministic repro artifacts or minimized cases instead of weakening transform contracts.