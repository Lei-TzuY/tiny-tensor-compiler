...
For an input shape `(d0, ..., dn)`, a non-full reduction removes every canonical reduced axis from the reduction result while preserving the relative order of unreduced axes. The dtype is unchanged.
...
1. build the ordinary deterministic reduction with its historical drop-axis result type and canonical `axis` metadata;
...
This keeps accumulation order, reduction code generation, OpenMP scheduling, and reduction serialization unchanged. It also means the retained-rank result participates naturally in the existing alias-lifetime verifier and broadcasting system. For example, reducing `(B, 3, 4)` over axes `(1, 2)` produces the ordinary `(B,)` reduction result and a verified `(B, 1, 1)` logical view over the same storage.

`keepdims=False` is the default and preserves the historical IR exactly: no extra attribute and no view are emitted. Equivalent keepdims expressions may therefore share both their canonical reduction and their identical view through existing exact CSE, while unused retained-rank results are removable by ordinary DCE.
...
Dimension retention does not change scheduling profitability or accumulator semantics. It is a logical rank policy composed after the reduction kernel.
...
Symbolic dimensions are fully specialized before Buffer/Loop lowering, exactly as for other operations. A reduced symbolic axis therefore becomes concrete before physical code generation. Under keepdims it is represented by extent `1`; an unreduced symbolic axis remains visible in the concrete retained-rank result shape for that specialization.

Reductions remain known-pure operations for DCE and exact CSE. Canonical axis metadata is part of expression identity, so equivalent axis collections may merge while different domains or different operators remain distinct. The keepdims view is itself an existing pure alias operation, so no reduction-specific optimizer path is required.

Canonical tuple attributes and the explicit view operation round-trip through the existing versioned tensor-IR serialization. Repro capture/replay therefore preserves retained-rank semantics without introducing a new reduction encoding.

## Deliberate boundaries

The current phase supports deterministic `sum`/`prod` over full, single-axis, or canonical multi-axis domains, with optional retained dimensions through verified view composition. Adding `min`, `max`, arg-reductions, arbitrary reducer callbacks, dtype-changing accumulation, unstable tree reassociation, parallel accumulator trees, or runtime-selected axes would require separate executable milestones and their own verifier/backend contracts. More `sum`/`prod` axis spelling variants or additional keepdims syntax are not roadmap goals.
