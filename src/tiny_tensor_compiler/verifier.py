...
def _verify_copy_into(op_index: int, op: Operation) -> None:
    ...
    if target.type.dtype != source.type.dtype:
        _fail(op_index, op, "copy_into target and source dtypes must exactly match")
    try:
        broadcast_type = infer_binary(target.type, source.type)
    except TypeInferenceError as exc:
        _fail(op_index, op, f"copy_into source is not broadcast-compatible: {exc}")
    if broadcast_type != target.type:
        _fail(op_index, op, "copy_into source must broadcast exactly to the target type")
